...
// ----------------------------------note the difference of radian and angle----------------------------------//
...
void BuildDescMat(const IplImage* xComp, // x gradient component
				  ...
				  const DescInfo descInfo, // parameters about the descriptor
				  CvMat* kernelMatrix) // kernel matrix of densely sampled angles
{
	// whether use full orientation or not
	float fullAngle = descInfo.fullOrientation ? (2 * M_PI) : (M_PI);
	...
	for(int i = 0; i < height; i++) {
		...
		for(int j = 0; j < width; j++, index++) {
			...
			if(descInfo.flagThre == 1 && magnitude <= descInfo.threshold) {
...
void HogComp(IplImage* img, DescMat* descMat, DescInfo descInfo, CvMat* kernelMatrix)
{
	int width = descMat->width;
	int height = descMat->height;
...
void HofComp(IplImage* flow, DescMat* descMat, DescInfo descInfo, CvMat* kernelMatrix)
{
	...
	BuildDescMat(xComp, yComp, descMat, descInfo, kernelMatrix);
	cvReleaseImage(&xComp);
	cvReleaseImage(&yComp);
}

void MbhComp(IplImage* flow, DescMat* descMatX, DescMat* descMatY, DescInfo descInfo, CvMat* kernelMatrix)
{
	...
	BuildDescMat(flowXdX, flowXdY, descMatX, descInfo, kernelMatrix);
	BuildDescMat(flowYdX, flowYdY, descMatY, descInfo, kernelMatrix);
...
int isValid(std::vector<CvPoint2D32f>& track, std::vector<float>& saliency, std::vector<float>& averageSaliency, float threshold)
{
	/*int size = track.size();
